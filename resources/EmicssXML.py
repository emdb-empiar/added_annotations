...
class EmicssXML:
    ...
    #                     self.emicss_empiar = self.EMICSS_empiar(val, samp_id, all_db, dbs, cross_ref_dbs)
    #                 if samp_id == "theoretical" or samp_id == "experimental":
    #                     self.emicss_weight = self.EMICSS_weight(val, samp_id, weights)
    #                 if samp_id == "PMC":
    #                     self.emicss_pmc = self.EMICSS_PMC(val, samp_id, all_db, dbs, cross_ref_dbs)
    #                 if samp_id == "GO":
    #                    cross_ref_dbs = self.EMICSS_GO(val, samp_id, all_db, dbs, cross_ref_dbs)
    #                 if (samp_id.isalnum() and not samp_id.isalpha() and not samp_id.isnumeric()):
    #                     if len(samp_id) == 4:
    #                         self.emicss_pdb = self.EMICSS_Pdbe(val, samp_id, all_db, dbs, weights)
    #                     if len(samp_id) != 4:
    #                         cross_ref_dbs = self.EMICSS_uniprot(val, samp_id, all_db, dbs, cross_ref_dbs)
    #                 if samp_id.isnumeric():
    #                     self.emicss_lig = self.EMICSS_ligands(val, samp_id, all_db, dbs, macromolecules)
    ...
    #
    # def EMICSS_PMC(self, val, samp_id, all_db, dbs, cross_ref_dbs):
    #     """
    ...
    #         cross_ref_db.set_accession_id("%s %s" % ("PMID:", pmedid))
    #         cross_ref_db.set_provenance("%s" % provenance)
    #         cross_ref_dbs.add_cross_ref_db(cross_ref_db)
    #     if pmcid:
    #         cross_ref_db = EMICSS.cross_ref_dbType()
    #         cross_ref_db.set_db_source("%s" % "PUBMED")
    #         cross_ref_db.set_accession_id("%s %s" % ("PMCID:", pmcid))
    #         cross_ref_db.set_provenance("%s" % provenance)
    #         cross_ref_dbs.add_cross_ref_db(cross_ref_db)
    #     if doi:
    #         cross_ref_db = EMICSS.cross_ref_dbType()
    #         cross_ref_db.set_db_source("%s" % "PUBMED")
    #         cross_ref_db.set_accession_id("%s %s" % ("DOI:", doi))
    #         cross_ref_db.set_provenance("%s" % provenance)
    #         cross_ref_dbs.add_cross_ref_db(cross_ref_db)
    #     if issn:
    #         cross_ref_db = EMICSS.cross_ref_dbType()
    #         cross_ref_db.set_db_source("%s" % "PUBMED")
    #         cross_ref_db.set_accession_id("%s %s" % ("ISSN:", issn))
    #         cross_ref_db.set_provenance("%s" % provenance)
    #         cross_ref_dbs.add_cross_ref_db(cross_ref_db)
    #     return cross_ref_dbs
    #
    ...
    #     return weights
    #
    # def EMICSS_uniprot(self, val, samp_id, all_db, dbs, cross_ref_dbs):
